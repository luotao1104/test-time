    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-900 to-purple-900 text-white">
      ...
      <main className="container mx-auto px-4 md:px-6 py-8 md:py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-4xl mx-auto">
          {games.map((game) => (
            <div key={game.id} className="bg-white/10 backdrop-blur-lg rounded-2xl p-5 md:p-6 border border-white/20">
              <h3 className="text-xl md:text-2xl font-bold mb-2">{game.name}</h3>
              <p className="text-sm md:text-base text-gray-300 mb-3 md:mb-4">{game.description}</p>
              <span className="inline-block px-3 md:px-4 py-1.5 md:py-2 bg-pink-500/30 rounded-full text-xs md:text-sm">
                {game.status}
              </span>
            </div>
          ))}
